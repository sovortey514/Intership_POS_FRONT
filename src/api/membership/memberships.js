...
export const fetchMembershipById = async (id, token) => {
  try {
    const response = await fetch(`${API_URL}/getmembershipById/${id}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        // Authorization: `Bearer ${token}`,
    ...
    if (!response.ok) {
      const errorMessage = await response.json();
      console.error("Error fetching Membership by ID:", errorMessage);
      throw new Error(errorMessage.message || "Failed to fetch Membership by ID.");
    }
